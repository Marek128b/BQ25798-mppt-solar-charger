...
\date{March 2026}
\newcommand{\versionnumber}{Version 2.1}
\newcommand{\shortname}{I2C Controlled, 1 to 4-Cell, 5-A Buck-Boost Battery Charger}
...
\begin{titlepage}
    ...
    \vfill
    \includegraphics[width=0.9\linewidth]{BQ25798 test board mppt V2.1 3d render noBG.png}
...
\begin{quote}
    Selection of input source can be made via I2C with \(EN_{ACDRV1}\) and \(EN_{ACDRV2}\) \cite{ti_bq25672_dual_input}.
\end{quote}
\begin{quote}
    ...
    the ACFETs-RBFETs are present, then updates the ACRB1-STAT or ACRB2-STAT status bits accordingly.
    The ACFET1-RBFET1 or ACFET2-RBFET2 can be controlled \textbf{by setting the register bit EN-ACDRV1 or
    EN-ACDRV2}. If the external ACFET-RBFET is not present, then tie VAC1 / VAC2 to VBUS and connect
    ACDRV1 / ACDRV2 to GND. The power MUX drivers support three different application cases, which are
    elaborated below. \cite{TI_BQ25798}
\end{quote}

\textbf{ACDRV Turn On Condition} \\
...
\begin{enumerate}
    \item The corresponding ACFET-RBFET was detected at power on: ACDRV is not short to ground.
    \item VAC is above \(V_{VBUSpresent}\)  threshold
    \item VAC is below VACOVP threshold
    \item DIS-ACDRV-BOTH is not set to '1'
    \item EN-HIZ is not set to '1'
    \item VBUS is below \(V_{VBUSpresent}\) threshold
\end{enumerate}
...
Example with \(\mathbf{V_{SYS} = 3.7V (1S-LiPo)}\) and \(\mathbf{V_{BUS} = 20V}\):
\begin{equation}
    I_{\text{RIPPLE\_BUCK}} = \frac{3.7V \times (20V - 3.7V)}{20V \times 750kHz \times 2.2\mu H} = 1.827A
    \label{equ:Inductor-buck-ripple-Example}
\end{equation}

Example with \(\mathbf{V_{SYS} = 14.8V (4S-LiPo)}\) and \(\mathbf{V_{BUS} = 3.6V}\):
\begin{equation}
    I_{\text{RIPPLE\_BOOST}} = \frac{14.8V \times (14.8V - 3.6V)}{14.8V \times 750kHz \times 2.2\mu H} = 6.788A
    \label{equ:Inductor-boost-ripple-Example}
\end{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.8\linewidth]{trace width calc.PNG}
...
\begin{equation}
    I_{\text{INDPM}} = \frac{\left( \frac{R_2}{R_1 + R_2} \cdot 5 \right) - 1}{0.8} \approx 0.975\,\text{A} = \textbf{975mA}
\end{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Screenshot 2025-12-12 111035.png}
    \caption{TS Resistor Network}
    \label{fig:TS-ResistorNetwork}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{defaultACDRVstatus.png}
    \caption{default ACDRV status}
    \label{fig:defaultACDRVstatus}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includesvg[width=\linewidth]{PCB_PCB_BQ25798-test-board-mppt-solar_2026-03-17.svg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textheight, angle=90]{Schematic_BQ25798-test-board-mppt-solar_2026-03-17.png}
    \caption{Schematics of the BQ25798 Test Board with MPPT Solar}
    \label{fig:schematics-v2}
\end{figure}
...
\subsection{Mistakes V2.0}
\begin{enumerate}
    \item VAC1 and VAC2 connected to VBUS instead of VIN1 and VIN2.
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{VAC1-VAC2 connected to VBUS wrong.png}
        \caption{VAC1 and VAC2 error in Schematics}
        \label{fig:VAC1-VAC2 connected to VBUS wrong}
    \end{figure}
\end{enumerate}

\subsection{V2.1 changes}
\begin{enumerate}
    \item Fixed VAC issues of V2.0
    \item changed R6 (NTC resistor Network) to right value of 4.3k\(\Omega\)
\end{enumerate}
